...
func doRetryForStatusCodesImpl(s Sender, r *http.Request, count429 bool, attempts int, backoff, cap time.Duration, codes ...int) (resp *http.Response, err error) {
	...
	for attempt := 0; attempt < attempts+1; {
		...
		resp, err = s.Do(rr.Request())
		if tre := UnwrapTokenRefreshError(err); tre != nil {
			// TokenRefreshError contains the underlying response and/or error from the
			// token refresh operation.  unpack them so we can apply retry logic to them.
			resp = tre.Response()
			err = tre.Unwrap()
		}
